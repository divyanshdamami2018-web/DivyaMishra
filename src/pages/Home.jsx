...
import { ShieldCheck, UserCheck, CalendarCheck, CreditCard, Quote, ArrowRight, Star } from 'lucide-react';
...
const HeroVisual = () => (
  <div className="relative w-[340px] h-[420px] lg:w-[420px] lg:h-[520px] mx-auto">
    {/* Outer glow */}
    <div className="absolute inset-0 bg-primary/20 rounded-full blur-3xl animate-pulse-slow" />
    {/* Rings */}
    <div className="hero-ring" style={{ inset: '-20px' }} />
    <div className="hero-ring" style={{ inset: '-50px', animationDelay: '1s' }} />
    {/* Main card */}
    <div className="glass-card absolute inset-0 rounded-[48px] overflow-hidden flex flex-col items-center justify-center p-8 text-center border-white/50 animate-float shadow-2xl">
      <div className="w-20 h-20 bg-gradient-to-br from-primary to-emerald-700 rounded-3xl flex items-center justify-center mb-6 shadow-xl">
        <span className="text-4xl">🌿</span>
      </div>
      <p className="text-primary font-bold text-xs tracking-widest uppercase mb-3">Your Safe Space</p>
      <h2 className="text-2xl font-bold text-slate-800 mb-3">Start Healing Today</h2>
      <p className="text-slate-500 text-sm leading-relaxed mb-6">
        Evidence-based therapy in a confidential, caring environment.
      </p>
      <div className="flex gap-2 flex-wrap justify-center">
        {['Anxiety', 'Depression', 'OCD', 'Trauma', 'Career'].map(tag => (
          <span key={tag} className="px-3 py-1 bg-primaryLight text-primary rounded-full text-xs font-bold">{tag}</span>
        ))}
      </div>
    </div>
    {/* Floating badge — top right */}
    <div className="absolute -top-4 -right-4 glass-card rounded-2xl px-4 py-3 shadow-xl border-white/60 flex items-center gap-2 animate-float" style={{ animationDelay: '1s' }}>
      <Star className="w-4 h-4 text-amber-400 fill-amber-400" />
      <span className="font-bold text-slate-800 text-sm">4.9 Rating</span>
    </div>
    {/* Floating badge — bottom left */}
    <div className="absolute -bottom-4 -left-4 glass-card rounded-2xl px-4 py-3 shadow-xl border-white/60 animate-float" style={{ animationDelay: '2s' }}>
      <p className="text-xs text-slate-500 font-bold">🔒 100% Confidential</p>
    </div>
  </div>
);
  ...
  return (
    <div className="flex flex-col min-h-screen font-sans">
      ...
      <section className="bg-mesh-gradient relative pt-20 pb-32 overflow-hidden px-4 md:px-0">
        <div className="max-w-7xl mx-auto flex flex-col lg:flex-row items-center justify-between z-10 relative gap-16">
          <div className="lg:w-1/2 space-y-8 animate-fade-in">
            ...
            <p className="text-xl text-slate-600 max-w-xl leading-relaxed">
              Therapy beyond traditional boundaries — a journey of self-discovery, resilience, and growth. Rooted in India's timeless wisdom and inspired by{' '}
              <span className="font-bold text-primary">Vasudhaiva Kutumbakam</span> — the world is one family.
            </p>
            <div className="flex flex-col sm:flex-row gap-4 pt-2">
              ...
              >
                ...
                <div className="absolute inset-0 bg-white/10 group-hover:translate-x-full transition-transform duration-500 -skew-x-12 translate-x-[-150%]" />
              </Link>
              <a
                href="#how-it-works"
        ...
        <div className="max-w-5xl mx-auto px-4 grid grid-cols-2 md:grid-cols-4 gap-8 text-center">
          {stats.map((s, i) => (
            <div key={i}>
              ...
              <p className="text-slate-500 font-medium text-sm">{s.label}</p>
            </div>
          ))}
        </div>
      </section>
      ...
      <section className="py-24 bg-white relative overflow-hidden">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex flex-col md:flex-row md:items-end justify-between mb-20 gap-8">
            ...
            <p className="text-slate-500 text-lg max-w-sm">World-class clinical support using evidence-based therapy in a calming digital ecosystem.</p>
          </div>
          ...
          <div className="glass-card rounded-[50px] overflow-hidden flex flex-col lg:flex-row shadow-2xl items-stretch border-none">
            <div className="lg:w-2/5 p-4">
              <img
                ...
                alt="Divya Mishra — Counseling Psychologist"
                className="w-full h-full object-cover rounded-[40px] shadow-lg min-h-[400px]"
                onError={(e) => {
                  e.target.style.display = 'none';
                  e.target.parentNode.innerHTML = '<div class="w-full min-h-[400px] rounded-[40px] bg-gradient-to-br from-primary/20 to-emerald-100 flex items-center justify-center"><span class="text-8xl">🌿</span></div>';
                }}
              />
            </div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-10 pt-4">
                <div className="space-y-5">
                  <h4 className="text-lg font-bold text-slate-800 flex items-center">
                    <span className="w-8 h-[2px] bg-primary mr-3" /> Qualifications
                  </h4>
                ...
                <div className="space-y-5">
                  <h4 className="text-lg font-bold text-slate-800 flex items-center">
                    <span className="w-8 h-[2px] bg-primary mr-3" /> Specialization
                  </h4>
                  <ul className="space-y-3 text-slate-600">
